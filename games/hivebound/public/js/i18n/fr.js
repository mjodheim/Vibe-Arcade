// The French layer. Hivebound is written in French first: this module holds
// every translation and is deliberately free of DOM and storage access, so the
// test suite can prove that nothing the player can read is left in English.

export const fr = {
  '2: +18% armour · 4: retaliation nova':'2 : +18 % d’armure · 4 : nova de riposte',
  '2: +18% damage · 4: kills can explode':'2 : +18 % de dégâts · 4 : les éliminations peuvent exploser',
  '2: +12% speed · 4: +22% crit damage':'2 : +12 % de vitesse · 4 : +22 % de dégâts critiques',
  '2: -12% cooldown · 4: ability cooldown rebounds to 1s after use':'2 : -12 % de recharge · 4 : la recharge retombe à 1 s après usage',
  '2: +24% score · 4: +48% score · Gloam itself empowers enemies':'2 : +24 % de score · 4 : +48 % de score · le Gloam renforce aussi les ennemis',
  'Username ≥ 3 chars and password ≥ 6 chars.':'Pseudo de 3 caractères minimum et mot de passe de 6 caractères minimum.',
  'Username already exists.':'Ce pseudo est déjà pris.','Invalid credentials.':'Identifiants invalides.',
  'Login required to submit a score.':'Connecte-toi pour envoyer un score.','Invalid run.':'Partie invalide.',
  'Invalid score.':'Score invalide.','Run validation failed.':'La validation de la partie a échoué.',
  'Unauthorized':'Non autorisé','Not found':'Introuvable','Internal server error':'Erreur interne du serveur',
  'Score':'Score','Space':'Espace','Shift':'Maj','Left Shift':'Maj gauche','Right Shift':'Maj droite','Ctrl':'Ctrl','Alt':'Alt','Esc':'Échap',
  'Relics of the Bloom':'Reliques de la Bloom','Bloom':'Bloom','Gloam':'Gloam','Thorn':'Thorn','Echo':'Echo','Wax':'Wax',
...
// Relic names are generated from two word lists, so they are translated by
// composition rather than one entry per combination. French puts the adjective
// after the noun and agrees it in gender, hence the two forms.
const RELIC_CORE = {
  Stinger:['Dard','m'], Carapace:['Carapace','f'], Petal:['Pétale','m'], Charm:['Charme','m'],
  Crown:['Couronne','f'], Vial:['Fiole','f'], Needle:['Aiguille','f'], Bell:['Cloche','f'],
  Lantern:['Lanterne','f'], Heart:['Cœur','m']
};
const RELIC_PREFIX = {
  Ancient:['ancien','ancienne'], Royal:['royal','royale'], Gilded:['doré','dorée'],
  Hollow:['creux','creuse'], Thornbound:['de ronces','de ronces'], Moonlit:['lunaire','lunaire'],
  Ashen:['cendré','cendrée'], Singing:['chantant','chantante'], Forbidden:['interdit','interdite'],
  Glass:['de verre','de verre']
};
const RARITY_CAPS = { COMMON:'COMMUN', RARE:'RARE', EPIC:'ÉPIQUE', LEGENDARY:'LÉGENDAIRE' };

function relicName(prefix, core){
  const noun = RELIC_CORE[core];
  const adjective = RELIC_PREFIX[prefix];
  if (!noun || !adjective) return null;
  return `${noun[0]} ${adjective[noun[1] === 'f' ? 1 : 0]}`;
}

// Returns the French form, or null when nothing here knows the string. The
// tests rely on that distinction: a word that is spelled the same in both
// languages is still "known", while a forgotten string is not.
function lookup(text){
  if(fr[text]!==undefined)return fr[text];
  let m;
  if((m=text.match(/^HTTP (\d+)$/)))return `Erreur HTTP ${m[1]}`;
  if((m=text.match(/^Survive (\d+)s$/)))return `Survivre ${m[1]} s`;
  if((m=text.match(/^Wave (\d+) \/ (\d+)$/)))return `Vague ${m[1]} / ${m[2]}`;
  if((m=text.match(/^Wave (\d+) of (\d+)$/)))return `Vague ${m[1]} sur ${m[2]}`;
  if((m=text.match(/^LV (\d+)$/)))return `NIV ${m[1]}`;
  if((m=text.match(/^Region (\d+)$/)))return `Région ${m[1]}`;
  if((m=text.match(/^Region (\d+) · Gloam (\d+)%$/)))return `Région ${m[1]} · Gloam ${m[2]} %`;
  if((m=text.match(/^The Guardian sheds its shell\. Phase (\d+)\.$/)))return `Le Gardien abandonne sa carapace. Phase ${m[1]}.`;
  if((m=text.match(/^Overflow (\d+): (.+)$/)))return `Débordement ${m[1]} : ${lookup(m[2]) ?? m[2]}`;
  if((m=text.match(/^\+(\d+)% damage · \+(\d+) vitality$/)))return `+${m[1]} % de dégâts · +${m[2]} de vitalité`;
  if((m=text.match(/^Choose (.+)$/)))return `Choisir ${lookup(m[1]) ?? m[1]}`;
  if((m=text.match(/^Guardian of (.+)$/)))return `Gardien de ${lookup(m[1]) ?? m[1]}`;
  if((m=text.match(/^REGION (\d+) · (.+)$/)))return `RÉGION ${m[1]} · ${lookup(toTitleCase(m[2])) ?? m[2]}`;
  if((m=text.match(/^The Gloam claimed a (.+)\.$/)))return `Le Gloam a eu raison de ${lookup(m[1]) ?? m[1]}.`;
  ...
  if((m=text.match(/^(.+) → (.+)$/)))return `${lookup(m[1]) ?? m[1]} → ${m[2]}`;
  if((m=text.match(/^(COMMON|RARE|EPIC|LEGENDARY) · (.+)$/)))return `${RARITY_CAPS[m[1]]} · ${translateCompound(m[2])}`;
  if((m=text.match(/^(Common|Rare|Epic|Legendary): (.+)$/)))return `${fr[m[1]]} : ${lookup(m[2]) ?? m[2]}`;
  if((m=text.match(/^\+(\d+)% damage$/)))return `+${m[1]} % de dégâts`;
  if((m=text.match(/^\+(\d+)% speed$/)))return `+${m[1]} % de vitesse`;
  if((m=text.match(/^\+(\d+)% attack speed$/)))return `+${m[1]} % de vitesse d’attaque`;
  if((m=text.match(/^\+(\d+)% crit$/)))return `+${m[1]} % de critique`;
  if((m=text.match(/^([A-Z][a-z]+) ([A-Z][a-z]+)$/))){const name=relicName(m[1],m[2]);if(name)return name}

  // A label shouted in capitals: "VERDANT REACH" is the same entry as
  // "Verdant Reach", shouted back.
  if(text===text.toLocaleUpperCase('fr-FR')&&/\p{L}{2}/u.test(text)){
    const sentence=lookup(toTitleCase(text));
    if(sentence!==null)return sentence.toLocaleUpperCase('fr-FR');
  }

  // A list of parts: translate each one that is known.
  if(text.includes(' · ')){
    const parts=text.split(' · ');
    const found=parts.map(part=>lookup(part));
    if(found.some(part=>part!==null))return parts.map((part,i)=>found[i]??part).join(' · ');
  }
  // An icon or a number in front of a label: "🌿 Region", "☠ Kills".
  if((m=text.match(/^([^\p{L}]+)(\p{L}.*)$/u))){const inner=lookup(m[2]);if(inner!==null)return `${m[1]}${inner}`}
  return null;
}
...
// True when the French layer knows what to do with this string — including
// strings that are spelled identically in both languages, and strings with
// nothing to translate at all.
export function hasTranslation(raw){
