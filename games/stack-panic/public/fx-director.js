...
(() => {
  ...
  function acidRain(ms=3600){const end=performance.now()+ms;const spawn=()=>{if(performance.now()>end)return;for(let i=0;i<4;i++){const life=rnd(1.2,2);particle({x:rnd(15,345),y:rnd(-80,-8),vx:rnd(-12,12),vy:rnd(260,420),life,max:life,size:rnd(3,7),type:'acid',color:'#b9ff66'});}if(Math.random()>.45){const x=rnd(25,335),y=rnd(240,660);later(()=>{burst(x,y,8,'#b9ff66',80);scorch(x,y,'#17330b');},rnd(180,520));}later(spawn,55);};spawn();}
  function sheepDust(){let i=0;const step=()=>{dust(rnd(40,320),rnd(430,660),18);if(++i<=11)later(step,260);};step();}
  function magnetStorm(ms=3500){const end=performance.now()+ms;const step=()=>{if(performance.now()>end)return;for(let i=0;i<8;i++){const left=Math.random()>.5,x=left?-8:368;particle({x,y:rnd(90,650),vx:left?rnd(180,310):rnd(-310,-180),vy:rnd(-30,30),life:.55,max:.55,size:rnd(2,5),type:'spark',color:i%2?'#43efff':'#8f7cff'});}later(step,100);};step();}
  ...
  function updateGrade(){const s=window.state;if(!s)return;const live=!!s.running&&!s.gameOver&&!s.paused;cabinet.classList.toggle('fx-hot',live&&s.chaos>=68);cabinet.classList.toggle('fx-vignette',live&&s.chaos>=86);}
  gradeTimer=setInterval(updateGrade,180);window.addEventListener('beforeunload',()=>clearInterval(gradeTimer));

  const baseTank=window.spawnTank;if(typeof baseTank==='function')window.spawnTank=function(){baseTank();[950,1650,2350,3050].forEach((t,i)=>later(()=>tracer(265,500-i*18,rnd(70,300),rnd(210,600),'#ffd166'),t));};
  ...
  const baseMagnet=window.startMagnet;if(typeof baseMagnet==='function')window.startMagnet=function(t){baseMagnet(t);magnetStorm(Math.max(2600,(t||performance.now()+3500)-performance.now()));};
