function scheduleNextEvent(now,recovery=0){
  ...
  // A heavy incident buys the player a breather before the next one.
...
function explodeBomb(b){
  const good=rng()<.55;
  for(let yy=-2;yy<=2;yy++)for(let xx=-2;xx<=2;xx++){const x=b.x+xx,y=Math.floor(b.y)+yy;if(x<0||x>=COLS||y<0||y>=ROWS)continue;const d=Math.abs(xx)+Math.abs(yy);if(d<=2){if(good)state.board[y][x]=0;else if(!state.board[y][x]&&rng()<.7)state.board[y][x]=6;}}
