document.addEventListener('keydown',e=>{
  ...
  if(state.mini){
    ...
    const lower=typeof e.key==='string'?e.key.toLowerCase():e.key;
    state.mini.keys[lower]=true;
...
soundBtn.addEventListener('click',()=>{
  ...
  if(state.sound){ensureAudio();if(state.running&&!state.musicTimer)musicTick();}
  else stopMusic();
});
...
cabinet.addEventListener('pointermove',e=>{
  ...
  if(state.mini.three&&window.Breach){window.Breach.steer(dx*.12,-dy*.12);}
    ...
    state.mini.playerX=clamp(state.mini.playerX+dx*.012,-4.8,4.8);
    state.mini.playerZ+=dy*.006;
  }
  touchStart={x:e.clientX,y:e.clientY};
});
