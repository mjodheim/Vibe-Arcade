...
cabinet.addEventListener('pointermove',e=>{if(!state.mini||!touchStart)return;const dx=e.clientX-touchStart.x,dy=e.clientY-touchStart.y;state.mini.playerX=clamp(state.mini.playerX+dx*.012,-4.8,4.8);state.mini.playerZ+=dy*.006;touchStart={x:e.clientX,y:e.clientY};});
cabinet.addEventListener('pointerup',()=>touchStart=null);

state.board=emptyBoard();state.next=makePiece();spawn();draw();requestAnimationFrame(update);
